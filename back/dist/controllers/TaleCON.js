...
class TalesClass {
    Create = async (req, res, next) => {
        try {
            ...
            const newTale = await dBase.query(QRY, values);
            return res
                .status(201)
                .json({
                success: true,
    ...
    FetchAll = async (req, res, next) => {
        try {
            ...
            const tales = await dBase.query(QRY);
            return res
                .status(201)
                .json({
                success: true,
    ...
    GetOne = async (req, res, next) => {
        try {
            ...
            const value = [id];
            const oneTale = await dBase.query(QRY, value);
    ...
    Update = async (req, res, next) => {
        try {
            ...
            const updatedTale = await dBase.query(QRY, values);
    ...
    Delete = async (req, res, next) => {
        try {
            ...
            const values = [id];
            const delTale = await dBase.query(QRY, values);
            return res
                .status(201)
                .json({
                success: true,
...
;
export const TALE = new TalesClass();
